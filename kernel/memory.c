...
#define MEM_BITMAP_BASE 0xc03fb000
/*************************************/
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    /*
     *   这里为什么是0x700000 ?
     *   used_mem后面会用于物理内存池的起始地址。  我们常说的高地址的1GB作为内核空间 是指 虚拟地址空间的高地址
     *   我们这里是将物理内存的低位 的  7mb ~ 23 mb的地址作为内核使用的物理地址，剩余的座位 用户空间用的物理地址
     *
     */

    uint32_t used_mem = 0x700000;
    uint32_t free_mem = all_mem - used_mem;
    uint16_t all_free_pages = free_mem / PG_SIZE;		  // 1页为4k,不管总内存是不是4k的倍数,
    // 对于以页为单位的内存分配策略，不足1页的内存不用考虑了。
    uint16_t kernel_free_pages = all_free_pages / 4;  // 1:3比例
...
void mem_init() {
    ...
    uint32_t mem_bytes_total = (*(uint32_t*)(0x703));
